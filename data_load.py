...
class DriveRecord(object):
    ...
    """
    def __init__(self, base_folder, index, file_name, steering_angle, crop_image=False, fake_image=False):
        """
        ...
        self.index = index
        self.center_file_name = full_file_name(base_folder, file_name)
        self.steering_angle = steering_angle
...
class DriveDataSet(object):
    ...
    @classmethod
    def from_tubgroup(cls, tubgroup_df, crop_images=False, fake_image=False,
                 filter_method=drive_record_filter_exclude_duplicated_small_angles):
        records = []
        for row in tubgroup_df.itertuples():
            # row = tubgroup.df.iloc[[index]].reset_index().values[0] # itertuples is much faster
            record = DriveRecord(None, row[0], file_name=row[1], steering_angle=row[2], crop_image=crop_images, fake_image=fake_image)
            records.append(record)

        records = DriveDataSet.drive_record_to_feeding_data(records, filter_method, all_cameras_images=False)
